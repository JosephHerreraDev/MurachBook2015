...
namespace FutureValue
{
	public partial class Form1 : Form
	{
		...
		private void btnCalculate_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (DataIsValid())
				{
					...
					if(IsCurrency(txtMonthlyInvestment, "Monthly Investment"))
					{ 
						decimal monthlyInvestment =
							Decimal.Parse(txtMonthlyInvestment.Text, NumberStyles.Currency);
						decimal interestRateYearly =
							Decimal.Parse(txtInterestRate.Text, NumberStyles.AllowDecimalPoint | NumberStyles.Any);
						int years =
							int.Parse(txtYears.Text, NumberStyles.Integer);
						...
						txtInterestRate.Text =  interestRateYearly.ToString() + " %";
		...
		{
			decimal number = 0m;
			if (Decimal.TryParse(textBox.Text, NumberStyles.AllowDecimalPoint | NumberStyles.AllowCurrencySymbol |
								 NumberStyles.AllowParentheses , CultureInfo.CurrentCulture, out number))
			{
				return true;
			}
			else
		...
		{
			decimal number = 0m;
			if (Decimal.TryParse(textBox.Text, NumberStyles.Any,
			...
			{
				MessageBox.Show(name + " is invalid.", "Entry Error");
				textBox.Focus();
				return false;
			}
		}
        ...
        {
            int number = 0;
            if (Int32.TryParse(textBox.Text,NumberStyles.Integer,	
							   CultureInfo.CurrentCulture, out number))
            {
                return true;
            }
        ...
        {
			decimal number = 
				(Decimal.Parse(textBox.Text, NumberStyles.Any));
